...
function results_show_stage_sections($atts) {
    ...
    foreach ($season_rows as $s) {
        ...
        $season_links .= '<a href="' . esc_url(add_query_arg(array(
            'season_id' => (int)$s['season_id'],
            'event_id' => $event_id,
            'class_name' => $class_name
        ), get_permalink())) . '" class="' . $active . '">' . esc_html($s['season_name']) . '</a>';
    }

    $class_links = '';
    foreach ($class_rows as $c) {
        $active = $c['class_name'] === $class_name ? ' class-filter-top3 active' : ' class-filter-top3';
        $class_links .= '<a href="' . esc_url(add_query_arg(array(
            'season_id' => $season_id,
            'event_id' => $event_id,
            'class_name' => $c['class_name']
        ), get_permalink())) . '" class="' . $active . '">' . esc_html($c['class_name']) . '</a>';
    }

    $event_links = '';
    foreach ($event_rows as $e) {
        $active = ((int)$e['event_id'] === $event_id) ? ' class-filter-top3 active' : ' class-filter-top3';
        $event_links .= '<a href="' . esc_url(add_query_arg(array(
            'season_id' => $season_id,
            'event_id' => (int)$e['event_id'],
            'class_name' => $class_name
        ), get_permalink())) . '" class="' . $active . '">' . esc_html($e['event_name']) . '</a>';
    }
...
<div class="results-info"><h3 style="font-size: 20px; margin-bottom: 20px; color: var(--text-main);">Таблица результатов <?php echo esc_html($event['event_name']); ?>  <?php echo $current_season_name; ?></h3></div>
...
<col class="col-section-cp"><col class="col-section-points"><col class="col-section-time"><col class="col-section-status">
<?php endforeach; ?>
<col class="col-total">
</colgroup>
  <thead>
   <tr>
    ...
    <?php foreach($sections as $s): ?><th colspan="3"><?php echo esc_html($s['section_name']); ?></th><?php endforeach; ?>
    <th rowspan="2" scope="col" class="scores-border">Всего<br>баллов</th>
    <th rowspan="2" scope="col" class="has-text-align-center">Место</th>
   </tr>
   <tr>
    <?php foreach($sections as $s): ?>
     <th scope="col">КП</th><th scope="col">Время</th><th scope="col">Баллы</th>
    <?php endforeach; ?>
   </tr>
  </thead>
  <tbody>
   <?php foreach($participants as $p): ?>
    <tr>
     ...
     <?php foreach($sections as $s):$r=$section_data[(int)$s['section_id']][$p['participant_id']]??null;$st=$r?($r['final_status']?:$r['status']):'';if($r):?><td><?php echo esc_html($r['checkpoints_count']); ?></td><td><?php echo esc_html($r['raw_time']?:'—'); ?></td>
     ...
     <td class="results_scores_all scores-border"><?php echo $p['has_finished']?esc_html($p['total']):'—'; ?></td><td class="results_place"><?php echo esc_html($p['place']); ?></td>
    </tr>
   <?php endforeach; ?>
  </tbody>
</table></figure></div>
...
function results_204_ajax_load_class() {
    ...
    echo results_show_stage_sections(array(
        'season_id' => $season_id,
        'event_id' => $event_id,
        'class_name' => $class_name,
        'render_scripts' => 0
